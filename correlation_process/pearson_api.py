...
def cbf():
    #csv_dir = r'E:\Samples\merge_avg_list\api_avg\apiAll.csv'
    csv_dir = r'E:\VS\api_process\api\apiAll.csv'
    pearson_dir = r'E:\VS\correlation_process\apiPea.csv'
    ...
    for i in range(0, len(features)-2):
        for j in range(i+1, len(features)-1):
            f_pearson[(features[i], features[j])] = pearsonr(dataset[features[i]], dataset[features[j]])[0]
            count += 1
            print("已完成", count)
    for key in f_pearson.keys():
        if f_pearson.get(key) >= 0.8:
