...
def transition_csv(avgList):
    ...
    avg_benign = pd.DataFrame(columns=avgList)
    # for n in range(len(permission_all)):
    avg_benign.to_csv(r'E:\VS\correlation_process\pea_benign.csv', encoding='utf-8', index=False)

def avg_csv():
    avgAVG_dir = r'E:\VS\correlation_process\featurePea.csv'
    root_dir = r'E:\VS\benign\all_non_repeat'
    avg_dir = r'E:\VS\correlation_process\pea_benign.csv'
    avgList = []
    count_all = []
    count = 0
